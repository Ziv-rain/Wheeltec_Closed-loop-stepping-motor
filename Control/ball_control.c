...
static uint8_t hard_limit(float p){return p<=PWM_LIMIT_LOW||p>=PWM_LIMIT_HIGH?1U:0U;}
...
static void htik(void){float p,e;if(hc==HC_READY||hc==HC_FAULT)return;if(hc==HC_INIT||hc==HC_WAIT){hm+=CL_PERIOD_MS;if(!Encoder_GetPwmAngle(ENCODER_AXIS_X,&p)){if(hm>=HOMING_WAIT_TIMEOUT_MS)hfail();return;}if(!sane(p)||hard_limit(p)){hfail();return;}if(absf(p-PWM_HORIZONTAL_REF)<=PWM_HORIZONTAL_TOL){hfin();}else{hstart(p);}return;}hm+=CL_PERIOD_MS;if(hm>=HOMING_MOVE_TIMEOUT_MS){hfail();return;}if(!Encoder_GetPwmAngle(ENCODER_AXIS_X,&p)){pmm+=CL_PERIOD_MS;if(pmm>=HOMING_PWM_LOSS_TIMEOUT_MS)hfail();return;}pmm=0;if(!sane(p)||hard_limit(p)){hfail();return;}e=absf(p-PWM_HORIZONTAL_REF);if(e<=PWM_HORIZONTAL_TOL){hfin();return;}if(e+0.05f<lherr){lherr=e;npm=0;}else{npm+=CL_PERIOD_MS;if(npm>=HOMING_NO_PROGRESS_MS){hfail();return;}}}
...
void BallControl_Tick5ms(void){
    ...
    if(hard_limit(pw)){stp();TaskCtrl_ReportFault();return;}
