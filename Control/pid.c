...
float PID_UpdateWithDerivative(PID_t*p,float error,float derivative,float dt){float o;if(dt<=0)dt=0.005f;p->integral=clampf(p->integral+error*dt,-p->integral_limit,p->integral_limit);o=p->kp*error+p->ki*p->integral+p->kd*derivative;p->prev_error=error;p->initialized=1;return clampf(o,p->output_min,p->output_max);}
