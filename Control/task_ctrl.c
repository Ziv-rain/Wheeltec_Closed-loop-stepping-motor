...
void UART1_IRQHandler(void){while(!DL_UART_Main_isRXFIFOEmpty(UART_1_INST)){uint8_t b=(uint8_t)DL_UART_Main_receiveData(UART_1_INST),t,l,*d;if(aa55(&rx,b,&t,&l,&d)){ts.lrx=tms;ts.link=1;if(t==0x40&&l==4)pushq(d[0],d[1],(int16_t)((uint16_t)d[2]|((uint16_t)d[3]<<8)));else if(t==0x42&&l==8){uint32_t pk=__get_PRIMASK();__disable_irq();w_enc_lp=w_enc_l;w_enc_rp=w_enc_r;w_enc_tsp=w_enc_ts;w_enc_l=(uint32_t)((uint16_t)d[0]|((uint16_t)d[1]<<8));w_enc_r=(uint32_t)((uint16_t)d[2]|((uint16_t)d[3]<<8));w_enc_ts=(uint32_t)d[4]|((uint32_t)d[5]<<8)|((uint32_t)d[6]<<16)|((uint32_t)d[7]<<24);if(w_enc_tsp&&w_enc_ts>w_enc_tsp){float dt=(float)(w_enc_ts-w_enc_tsp)/1000.0f;int32_t dl32=(int32_t)(w_enc_l-w_enc_lp);int32_t dr32=(int32_t)(w_enc_r-w_enc_rp);if(dt>=0.01f&&dt<=0.5f&&dl32>-500&&dl32<500&&dr32>-500&&dr32<500){float dl=(float)dl32*WENC_SCALE;float dr=(float)dr32*WENC_SCALE;float sp=(dl+dr)*0.5f/dt;float a=(sp-w_speed_ms)/dt;if(a>-3.0f&&a<3.0f){w_speed_ms=sp;w_accel_ms2=a;w_valid=1U;}}}if(!pk)__enable_irq();}}}}
uint8_t TaskCtrl_GetWheelAccel(float*ax_mps2){if(!ax_mps2||!w_valid)return 0U;*ax_mps2=w_accel_ms2;return 1U;}
