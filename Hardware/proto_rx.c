...
typedef struct{uint8_t s,t,l,i,d[RX_MAX];uint16_t c;volatile BallData_t b;volatile float tgt;volatile uint8_t new_tgt;}PR_t;
static PR_t r;
...
void ProtoRx_Init(void){r.s=0;r.b.age_ms=0xFFFFFFFFU;r.b.frame_id=0U;r.b.velocity_centi_cm_s=0;r.new_tgt=0U;DL_UART_Main_enableInterrupt(UART_2_INST,DL_UART_MAIN_INTERRUPT_RX);}
void ProtoRx_ProcessByte(uint8_t b){uint16_t c;switch(r.s){case 0:if(b==0xAA)r.s=1;break;case 1:if(b==0x55)r.s=2;else r.s=(b==0xAA)?1:0;break;case 2:r.t=b;r.s=3;break;case 3:r.l=b;r.i=0;if(r.l>RX_MAX){r.s=0;break;}r.s=r.l?4:5;break;case 4:r.d[r.i++]=b;if(r.i>=r.l)r.s=5;break;case 5:r.c=b;r.s=6;break;default:c=(uint16_t)r.c|((uint16_t)b<<8);r.s=0;if(c==crc16f(r.t,r.l,r.d)){if(r.t==0x30&&r.l==8){uint32_t pk=__get_PRIMASK();__disable_irq();r.b.position_centi_cm=(int16_t)((uint16_t)r.d[0]|((uint16_t)r.d[1]<<8));r.b.confidence=r.d[2];r.b.status=r.d[3];r.b.velocity_centi_cm_s=(int16_t)((uint16_t)r.d[4]|((uint16_t)r.d[5]<<8));r.b.age_ms=0;r.b.frame_id++;if(r.b.frame_id==0U)r.b.frame_id=1U;if(!pk)__enable_irq();}else if(r.t==0x31&&r.l==8){uint32_t pk=__get_PRIMASK();__disable_irq();r.tgt=(float)((int16_t)((uint16_t)r.d[0]|((uint16_t)r.d[1]<<8)))/100.0f;r.new_tgt=1U;if(!pk)__enable_irq();}}break;}}
...
uint8_t ProtoRx_GetTouchTarget(float*target_cm){uint32_t pk=__get_PRIMASK();__disable_irq();if(!target_cm||!r.new_tgt){if(!pk)__enable_irq();return 0U;}*target_cm=r.tgt;r.new_tgt=0U;if(!pk)__enable_irq();return 1U;}
